...
- Controles separados: parcelamento dentro do cartão de ajuste das formas de pagamento, desconto Pix em cartão próprio. Cada um usa `evaluateActions()` isoladamente para não atribuir a uma medida o efeito da outra. O cartão de liquidação de estoque permite escolher parte do estoque e desconto, mas apenas compõe um plano de campanha, sem estimar caixa porque `dist/engine.js` não implementa ação de promoção.
...
- Liquidação de estoque aguarda contrato analítico para estimar impacto sobre vendas, margem, estoque remanescente e caixa. Os percentuais escolhidos no front são apenas um rascunho de campanha.
