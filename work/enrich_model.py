...
unc=montecarlo.simular_cenarios(forecast,resid,mix,concentracao,rates,delays,receivables,exp_total,6500,data['dates'],diagnosis['entradas90'],diagnosis['saidas90'],rng,parcelas=3)
...
new={'id':'novo','name':'Café Primeiro Passo','sector':'Alimentação','age':1,'balance':6500,'mix':mix,'rates':rates,'delays':delays,'installments':3,'history':[{'date':d.isoformat(),'sales':round(float(v),2)} for d,v in zip(histdates,y)],'forecast':forecast.round(2).tolist(),'receivables':receivables.round(2).tolist(),'expenses':expenses,'coldStart':True,'peerCount':18,'trainSamples':len(X),'historyDays':14,'metrics':None,'uncertainty':unc,'diagnosis':diagnosis,'importance':[{'name':n,'value':round(float(v),4)} for n,v in zip(['Dia da semana','Dia do mês','Mês','Média de 7 dias','Média de 14 dias','Variação de 14 dias'],model.feature_importances_)]}
